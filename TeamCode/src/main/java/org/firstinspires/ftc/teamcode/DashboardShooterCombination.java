...
public class DashboardShooterCombination extends OpMode {
    ...
    // Kicker Settings
    public static double KICKER_POS_ACTIVE = 0.7;  // "Up" position
    public static double KICKER_POS_IDLE = 1.0;    // "Down" position
    public static double KICKER_DELAY_MS = 1000;   // Time to stay up (1 second)

    // Spin Servo Positions
    public static double SPIN_RETRACTED = 0.0;     // dpad_down
    public static double SPIN_MID = 0.45;           // dpad_right
    public static double SPIN_MAX = 0.87;           // dpad_up
    public static double SPIN_CUSTOM_1 = 0.68;      // dpad_left
    public static double SPIN_CUSTOM_2 = 0.25;      // gamepad X
    public static double SPIN_CUSTOM_3 = 1.0;      // gamepad Y
    ...
    public void init() {
        ...
        try {
            ...
            currentSpinTarget = SPIN_RETRACTED;
    ...
    public void loop() {
        ...
        // --- 2. TIMED KICKER SERVO (Gamepad 1 - Right Trigger) ---
        // Start the kick
        ...
        // Automatic retraction after delay
        if (kickerIsActive && kickerTimer.milliseconds() > KICKER_DELAY_MS) {
            kick_servo.setPosition(KICKER_POS_IDLE);
            kickerIsActive = false;
        }

        // Manual override/reset (Gamepad 1 - Left Trigger)
        if (gamepad1.left_trigger > 0.5) {
            kick_servo.setPosition(KICKER_POS_IDLE);
            kickerIsActive = false;
        }

        // --- 3. SPIN SERVO (D-pad & Buttons) ---
        ...
        // --- 4. TELEMETRY ---
        double actualRPM = (shooterMotor.getVelocity() * 60.0) / TICKS_PER_REV;
        ...
        telemetry.addData("Kicker Timer", "%.1f s", kickerTimer.seconds());
        telemetry.addData("Spin Target", "%.2f", currentSpinTarget);
        telemetry.update();
    }
}