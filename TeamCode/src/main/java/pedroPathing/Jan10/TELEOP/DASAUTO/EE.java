...
@Autonomous(name = "Blue AutoPPG", group = "Main")
public class EE extends OpMode {
    ...
    private static final ShootState[][] SHOOT_ORDERS = {
            { ShootState.A, ShootState.B, ShootState.C }, // Cycle 1
            { ShootState.A, ShootState.B, ShootState.C }, // Cycle 2
            { ShootState.B, ShootState.A, ShootState.C } // Cycle 3
    };
    ...
    public void loop() {
        ...
        telemetry.addData("Shoot Cycle", shootCycleIndex);
    ...
    private void startShoot(double rpm, double hood) {
        ...
        int cycle = Math.min(shootCycleIndex, SHOOT_ORDERS.length - 1);
        shootState = SHOOT_ORDERS[cycle][shotIndex];
        shootCycleIndex++;
    ...
    private void advanceShot() {
        ...
        int cycle = Math.min(shootCycleIndex - 1, SHOOT_ORDERS.length - 1);
        shootState = SHOOT_ORDERS[cycle][shotIndex];
        shootTimer.reset();
    }
